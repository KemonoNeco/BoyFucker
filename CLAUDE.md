...
`boyfucker` — a personal Discord moderation bot (Rust). It connects to the gateway, logs "Logged in as …", and serves six moderation slash commands: `/purge`, `/kick`, `/ban`, `/unban`, `/mute` (timeout), `/unmute`, plus two general-purpose ungated commands: `/poll` (native Discord poll) and `/join` (join a voice channel — presence-only via songbird, no audio). An LLM integration may follow.
...
- **Voice is songbird, gateway-only.** `songbird` is pulled in with `default-features = false, features = ["gateway", "serenity", "rustls"]` — deliberately **without** the `driver` feature, so the native opus/symphonia audio toolchain is not a build dependency. The manager is registered on the serenity `ClientBuilder` via `SerenityInit::register_songbird()` (stored in serenity's typemap, *not* on `Data`), retrieved in handlers with `songbird::get(ctx).await`. `/join` uses `join_gateway` (presence only — the bot appears in the channel, transmits no audio); the driver-gated `join` is unavailable in this configuration. **Add the `driver` feature only when a feature needs real audio playback** (it requires a C toolchain/libopus).
